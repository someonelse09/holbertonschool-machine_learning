...
def convolve_channels(images, kernel, padding='same', stride=(1, 1)):
    ...
    if isinstance(padding, tuple) and len(padding) == 2:
        ...
        # For 'same' padding, use proper asymmetric padding for even kernels
        ph = (kh - 1) // 2
        pw = (kw - 1) // 2

        # Handle even kernels with asymmetric padding
        ph_extra = (kh - 1) % 2
        pw_extra = (kw - 1) % 2

        # Apply asymmetric padding (extra at beginning for consistency)
        padded_images = np.pad(images, ((0, 0), (ph + ph_extra, ph), (pw + pw_extra, pw), (0, 0)), mode='constant')
    elif padding == 'valid':
        ph, pw = 0, 0
