...
class NST:
    ...
    def total_cost(self, generated_image):
        ...
        j_variational = self.variational_cost(generated_image)

        # Calculate total cost: J = alpha * J_content + beta * J_style
        j_total = self.alpha * j_content + self.beta * j_style + self.var * j_variational

        return j_total, j_content, j_style, j_variational
    ...
    def generate_image(self, iterations=1000,
        ...
        Args:
            ...
            print Cost at iteration {i}: {J_total},
             content {J_content}, style {J_style}, var {J_var}
            i is the iteration
    ...
    def variational_cost(generated_image):
        ...
        Args:
            generated_image - a tf.Tensor of shape
        ...
        a = tf.square(generated_image[:, :, :-1, :] - generated_image[:, :, 1:, :])
        # Vertical differences
        b = tf.square(generated_image[:, :-1, :, :] - generated_image[:, 1:, :, :])
        # Summing all differences
        variational_cost = tf.reduce_sum(b) + tf.reduce_sum(a)

        return variational_cost
