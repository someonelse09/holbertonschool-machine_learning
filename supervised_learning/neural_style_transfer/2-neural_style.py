...
class NST:
    ...
    def gram_matrix(input_layer):
        ...
        Args:
            ...
            if input_layer is not an instance of tf.Tensor or
             tf.Variable of rank 4, raise a TypeError with
              the message input_layer must be a tensor of rank 4
        Returns:
             a tf.Tensor of shape (1, c, c)
              containing the gram matrix of input_layer
        """
        if not isinstance(input_layer, (tf.Tensor, tf.Variable)):
            raise TypeError("input_layer must be a tensor of rank 4")
        if input_layer.shape != 4:
            raise TypeError("input_layer must be a tensor of rank 4")

        batch, height, width, channels = input_layer.shape
        # Reshape the input layer to (h*w, c)
        # This combines spatial dimensions (h, w) into one dimension
        features = tf.reshape(input_layer, [height*width, channels])

        # Calculate the gram matrix
        # Gram matrix G_ij = sum over all positions of F_i * F_j
        # Using matrix multiplication: G = F^T * F
        # where F is the reshaped features matrix
        gram = tf.matmul(features, features, transpose_a=True)

        # Normalize by the number of locations (h * w)
        gram = gram / tf.cast(height*width, tf.float32)

        # Add batch dimension back to get shape (1, c, c)
