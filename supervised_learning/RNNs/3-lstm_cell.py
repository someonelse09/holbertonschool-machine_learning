...
class LSTMCell:
    ...
    def forward(self, h_prev, c_prev, x_t):
        ...
        # Forget gate: decides what
        # information to discard from cell state
        # sigmoid(concat @ Wf + bf):
        # (m, h+i) @ (h+i, h) -> (m, h)
        f_t = 1 / (1 + np.exp(-(concat @ self.Wf + self.bf)))

        # Input/Update gate: decides what
        # new information to store in cell state
        # sigmoid(concat @ Wu + bu):
        # (m, h+i) @ (h+i, h) -> (m, h)
